...
def simulate_failure(
    ...
    # ============================================================
    # BOUNDED DISRUPTION COST CALCULATION
    # ============================================================
    # Instead of raw: cost = sum(base_cost * impact_score)
    # Use: disruption_cost = sum(base_cost * min(impact_score, 0.3x disruption_factor))
    # where disruption_factor = 0.1 to 0.3 (realistic operational impact)
    # ============================================================
    disruption_factor = 0.15  # ~15% of affected supplier base cost is realistic disruption
    disruption_cost = 0.0
    for row in impacted_rows:
        affected_supplier = row["name"]
        impact = float(row.get("impact_score", 0.0))
        # Cap impact at 1.0 for multiplication (unbounded score -> bounded cost)
        capped_impact = float(np.clip(impact, 0.0, 1.0))
        affected_cost = float(base_cost.get(affected_supplier, 0.0))
        # Disruption cost = (cost of affected supplier) * (capped impact) * (disruption factor)
        disruption_cost += affected_cost * capped_impact * disruption_factor

    # Normalize potential_cost_increase as percentage of total_spend (max ~30%)
    if total_spend > 0:
        cost_increase_percent = (100.0 * disruption_cost / total_spend)
        # Cap percentage at 30% for realism (no 700% jumps)
        cost_increase_percent = float(np.clip(cost_increase_percent, 0.0, 30.0))
        potential_cost_increase = disruption_cost
    else:
        potential_cost_increase = 0.0
        cost_increase_percent = float("nan")

    # Alternative: keep both absolute and normalized for detailed reporting
    # potential_cost_increase = disruption_cost
    # cost_increase_percent = (100.0 * disruption_cost / total_spend) if total_spend > 0 else 0.0
