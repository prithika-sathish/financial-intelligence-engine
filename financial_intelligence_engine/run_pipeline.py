...
def run_pipeline() -> dict:
    ...
    stage_started = time.perf_counter()
    LOGGER.info("Stage 9b/11 start: workflow - recommendations and alerts")
    # Enrich predictions with context-aware recommendations
    predictions_df = enrich_predictions_with_recommendations(predictions_df)
    
    # Track portfolio state
    portfolio_df = track_portfolio_state(predictions_df, output_dir=root / "outputs")
    portfolio_summary = get_portfolio_summary(portfolio_df)
    
    # Send alerts for top N risky suppliers (with email simulation for safety)
    alert_results = send_top_supplier_alerts(
        predictions_df,
        top_n=5,
        simulate=False,  # Real emails enabled - update SMTP env vars to send
    )

    sent_count = sum(1 for r in alert_results if r.get("method") == "email" and r.get("sent"))
    failed_count = sum(1 for r in alert_results if r.get("method") == "failed" or not r.get("sent", False))
    logged_count = sum(1 for r in alert_results if r.get("method") == "logged")
    failure_reasons = [str(r.get("error")) for r in alert_results if r.get("method") == "failed" and r.get("error")]
    LOGGER.info(
        "Workflow email outcomes | sent=%d failed=%d logged=%d total=%d",
        sent_count,
        failed_count,
        logged_count,
        len(alert_results),
    ...
    if failure_reasons:
        LOGGER.warning("Workflow email failure reasons: %s", failure_reasons[:3])
    
    LOGGER.info(
        "Stage 9b/11 end: workflow complete | alerts_sent=%d portfolio_suppliers=%d duration_sec=%.3f",
        len(alert_results),
        len(portfolio_df),
        time.perf_counter() - stage_started,
    )
    ...
    # Portfolio state already saved by track_portfolio_state, but save summary
    portfolio_summary_path = output_dir / "portfolio_summary.json"
    with portfolio_summary_path.open("w", encoding="utf-8") as fh:
        json.dump(portfolio_summary, fh, indent=2)
...
if __name__ == "__main__":
    result = run_pipeline()
    alerts_count = len(result.get("alert_results", []))
    portfolio_high_risk = result.get("portfolio_summary", {}).get("high_risk_count", 0)
    LOGGER.info(
        "Pipeline completed. Alerts generated: %s | High-risk suppliers: %s | Agent alerts: %s",
        alerts_count,
        portfolio_high_risk,
        len(result["agent"].get("alerts", [])),
    )
