...
def _normalize_supplier(
    ...
    # Extract cost components (new: replacement, delay, dependency)
    replacement_cost = _to_float(row.get("replacement_cost"), 0.0)
    delay_cost = _to_float(row.get("delay_cost"), 0.0)
    dependency_cost = _to_float(row.get("dependency_cost"), 0.0)
    cost_explanation = str(row.get("cost_explanation", "")).strip() if row.get("cost_explanation") else ""
...
def _render_impact(supplier: dict[str, Any], graph: dict[str, Any]) -> None:
    ...
    replacement = impact.get("replacement_cost")
    dly_cost = impact.get("delay_cost")
    dep_cost = impact.get("dependency_cost")
    cost_explain = impact.get("cost_explanation", "")
    ...
    if replacement is not None or dly_cost is not None or dep_cost is not None:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.markdown("<div class='section-title'>Cost Impact Breakdown</div>", unsafe_allow_html=True)
        breakdown_cols = st.columns(3)
        with breakdown_cols[0]:
            st.metric("Replacement Cost", f"{_format_short(replacement, 2) if replacement is not None else 'N/A'}")
        with breakdown_cols[1]:
            st.metric("Delay Cost", f"{_format_short(dly_cost, 2) if dly_cost is not None else 'N/A'}")
        with breakdown_cols[2]:
            st.metric("Dependency Exposure", f"{_format_short(dep_cost, 2) if dep_cost is not None else 'N/A'}")
        
        if cost_explain:
            st.markdown(f"**Reason:** {cost_explain}")
        st.markdown("</div>", unsafe_allow_html=True)
