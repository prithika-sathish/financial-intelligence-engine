...
def _render_top_cost_impact_section(data: dict[str, pd.DataFrame]) -> None:
    st.header("Section 14 - Top Cost Impact Risks")
    pred = data["predictions"].copy()
    ...
    for col in ["company_id", "propagated_risk", "estimated_cost_impact", "recommended_action"]:
        if col not in pred.columns:
    ...
    pred["propagated_risk"] = pd.to_numeric(pred["propagated_risk"], errors="coerce").fillna(0.0)
    pred["estimated_cost_impact"] = pd.to_numeric(pred["estimated_cost_impact"], errors="coerce").fillna(0.0)

    top = pred.sort_values("estimated_cost_impact", ascending=False).head(10)

    fig = px.bar(
        top,
        x="company_id",
        y="estimated_cost_impact",
        color="propagated_risk",
        color_continuous_scale=["#10b981", "#f59e0b", "#ef4444"],
        title="Top Suppliers by Estimated Cost Impact",
        hover_data=["recommended_action"],
    )
    fig.update_layout(template="plotly_dark", xaxis_title="Supplier/Company", yaxis_title="Estimated Cost Impact")
    st.plotly_chart(fig, width="stretch")

    st.dataframe(
        top[["company_id", "propagated_risk", "estimated_cost_impact", "recommended_action"]],
        hide_index=True,
        width="stretch",
    )


def _render_failure_simulation_section(data: dict[str, pd.DataFrame]) -> None:
    st.header("Section 15 - Failure Simulation")
    pred = data["predictions"].copy()
    ...
    for col in ["company_id", "potential_impact_nodes", "potential_cost_increase", "suggested_alternatives"]:
        if col not in pred.columns:
            pred[col] = "" if col in ["potential_impact_nodes", "suggested_alternatives"] else 0.0

    suppliers = sorted(pred["company_id"].dropna().astype(str).unique().tolist())
    selected = st.selectbox("Select supplier", suppliers) if suppliers else None
    if not selected:
        st.info("No suppliers available for simulation view.")
        return

    row = pred[pred["company_id"].astype(str) == str(selected)].head(1)
    if row.empty:
        st.info("Supplier not found in predictions.")
        return

    picked = row.iloc[0]
    impacted = _safe_literal(picked.get("potential_impact_nodes"), [])
    if not isinstance(impacted, list):
        impacted = []

    cost_increase = float(pd.to_numeric(picked.get("potential_cost_increase", 0.0), errors="coerce") or 0.0)
    alternatives_raw = str(picked.get("suggested_alternatives", "") or "")
    alternatives = [x.strip() for x in alternatives_raw.split("|") if x.strip()]

    c1, c2 = st.columns(2)
    c1.metric("Potential Cost Increase", f"{cost_increase:,.2f}")
    c2.metric("Impacted Nodes", len(impacted))

    st.subheader("Impacted Nodes")
    if impacted:
        st.dataframe(pd.DataFrame({"impacted_node": impacted}), hide_index=True, width="stretch")
    else:
        st.info("No materially impacted nodes from simulated failure.")

    st.subheader("Suggested Alternatives")
    if alternatives:
        st.dataframe(pd.DataFrame({"alternative_supplier": alternatives[:3]}), hide_index=True, width="stretch")
    else:
        st.info("No low-risk structural alternatives identified.")
...
def main() -> None:
    ...
    if "Top Cost Impact Risks" in selected_sections:
        _render_top_cost_impact_section(data)
    if "Failure Simulation" in selected_sections:
        _render_failure_simulation_section(data)
